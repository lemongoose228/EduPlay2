...
| Frontend | React 19, TypeScript, Vite 8, Redux Toolkit, React Router, axios, socket.io-client, React Compiler (`babel-plugin-react-compiler`) |
...
| `station` | Станции | Однопользовательский сценарий | Линейный маршрут из 3–20 станций (название, задание, опциональное изображение, форма и цвет узла). Ведущий отмечает успех или провал; после прохождения всех станций неуспешные проходятся повторно. На экране игры маршрут рисуется SVG-зигзагом (`StationGamePage`, `stationPathUtils`). |
...
**Настройки шаблона:** таймеры на вопрос или термин, отрицательные очки (своя игра), а также параметры, специфичные для типа — категории и очки, варианты ответов, термины, секции колеса, список станций с линейными связями (конструктор `StationGameBuilder`, без React Flow), ровно 9 вопросов для крестиков-ноликов.

**Конструктор «Станции»:** форма со списком станций (название, задание, картинка, форма узла, цвет). Связи маршрута строятся автоматически между соседними станциями (`buildStationConnections`). Визуальный редактор графа не используется.
